...
class LANServer:
            ...
            try:
                ...
                if packet:
                    ...
                    if packet.stream_id == 0:
                        ...
                        if sender_username:
                            logger.debug(f"Received UDP packet from {sender_username}: seq={packet.seq_num}")
    ...
    def _learn_udp_address(self, stream_id: int, address: tuple) -> str:
        ...
        with self.clients_lock:
            for username, client in self.clients.items():
                # Check if this stream ID belongs to this client
                # We need to match stream IDs generated by the client
                # For now, we'll use a simple approach: learn from any packet
                if client.udp_address is None:
    ...
    def _relay_udp_packet(self, packet_data: bytes, sender_address: tuple, sender_username: str):
        ...
        if relayed_count > 0:
            logger.debug(f"Relayed packet from '{sender_username}' to {relayed_count} clients")
